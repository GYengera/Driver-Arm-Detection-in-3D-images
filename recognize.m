...
for i = 1:240
    for j = 1:320
        if prediction(320*(i-1) + j)>0.9 && Darray(i,j) > 0   %% Threshold of 0.9 is used
            X(i,j) = 1800;
        else 
            X(i,j) = Darray(i,j);
        end
...
for i = 1:240
    for j = 1:320
        if prediction(320*(i-1) + j)>0.9 && Darray(i,j) > 0
            X(i,j) = 0;
        else 
            X(i,j) = X(i,j) + 300;
        end
